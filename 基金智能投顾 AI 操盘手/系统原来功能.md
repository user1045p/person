### 🔧 后端 API 功能（Worker 路由处理）

| 路由                                             | 功能说明                                                     |
| :----------------------------------------------- | :----------------------------------------------------------- |
| `GET /api/indices`                               | 获取四大指数的实时价格、涨跌额、涨跌幅（来自东方财富）       |
| `GET /api/ranking?type=up|down&limit=n`          | 获取收盘净值涨跌排行榜（东方财富 rankhandler 接口）          |
| `GET /api/ranking/estimate?type=up|down&limit=n` | 获取盘中实时估值排行榜（先获取热门基金代码，再并发获取天天基金估值，排序后返回） |
| `GET /api/fund/estimate?code=xxxxxx`             | 获取单只基金的实时估值、估算涨跌幅、估值时间、净值日期       |
| `GET /api/fund/:code/detail`                     | 获取基金基本信息（全称、类型、规模、评级、经理、费率等）     |
| `GET /api/fund/:code/portfolio`                  | 获取基金持仓股票（代码、名称、占比、行业）及报告期           |
| `GET /api/fund/:code/performance`                | 获取阶段收益（周/月/季/年/成立以来）及同类排名               |
| `GET /api/fund-list`                             | 返回完整的基金列表（来自项目内置的 `fund-list.json` 文件）   |
